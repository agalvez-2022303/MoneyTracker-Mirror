...
async function verificarEmailGoogle(idToken: string): Promise<string> {
...
export async function loginConGoogle(idToken: string): Promise<authService.ResultadoLogin> {
  ...
  const email = await verificarEmailGoogle(idToken);
  ...
  if (existente) {
    ...
    usuario = await usuariosService.crearUsuarioOAuth(email);
  }
